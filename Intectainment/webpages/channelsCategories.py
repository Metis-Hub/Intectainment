...
from Intectainment.datamodels import Channel, Category, Post, RssFeeds, User, Rss_link
    ...
    if request.method == "POST":
        ...
        if rss_url:
            ...
            if Rss_link.query.filter_by(url=rss_url).first():
                Rss_link.query.filter_by(url=rss_url).channel.append(Channel.query.filter_by(name=name).first())
            else:
